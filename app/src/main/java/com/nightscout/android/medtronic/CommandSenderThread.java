...
public class CommandSenderThread implements Runnable {
    ...
    public CommandSenderThread(MedtronicReader reader, byte[] idPump, DataSource dataSource, Handler mHandler4) {
        this.reader = reader;
        this.idPump = idPump;
     ...
     */
    public CommandSenderThread(byte[] commandList, ArrayList<Messenger> mClients, MedtronicReader reader, byte[] idPump, DataSource dataSource, Handler mHandler4) {
    ...
    protected int sendMedtronicPumpRequest(byte command, byte repeat) {
        try {
            if (idPump != null && idPump.length > 0) {
                ...
                int resultWrite = mDataSource.write(readSystemTime);
                return resultWrite;
            }
        } catch (Exception e) {
            StringBuffer sb1 = new StringBuffer("");
